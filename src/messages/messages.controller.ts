...
import { ContactId, MessageTypes } from '@open-wa/wa-automate';
...
export class MessagesController {
  ...
  async sendMessage(@Body() message: InsertMessageDTO): Promise<string> {
    ...
    const checkNumber = await session.checkNumberStatus(
      `${Number(message.contact)}@c.us` as ContactId,
    );
    ...
    return JSON.stringify({
      message:
        'An error ocurred while saving the message. Check the server logs.',
    });
  }
}
